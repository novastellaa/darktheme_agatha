...
export default function PromptDialog({
    ...
    const [tempTitle, setTempTitle] = useState(editingPrompt?.title || '');
    const [tempPrompt, setTempPrompt] = useState(editingPrompt?.prompt || '');
    const [tempModel, setTempModel] = useState(editingPrompt?.model || 'gpt-3.5-turbo');
    const [tempTemperature, setTempTemperature] = useState(editingPrompt?.temperature || 0.5);
    const [tempTopP, setTempTopP] = useState(editingPrompt?.topP || 1);
    const [tempPresencePenalty, setTempPresencePenalty] = useState(editingPrompt?.presencePenalty || 0);
    const [tempFrequencyPenalty, setTempFrequencyPenalty] = useState(editingPrompt?.frequencyPenalty || 0);
    const [tempMaxTokens, setTempMaxTokens] = useState(editingPrompt?.maxTokens || 2048);
            ...
            <DialogContent className="sm:max-w-[700px] max-h-[98vh] flex flex-col">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-bold">
                        {editingPrompt ? 'Edit Prompt' : 'Custom Prompt and Settings'}
                    </DialogTitle>
                </DialogHeader>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={tempModel} onValueChange={setTempModel}>
                                <SelectTrigger className="w-full">
                                    <SelectValue placeholder="Select a model" className="bg-gray" />
                                </SelectTrigger>
                                <SelectContent>
